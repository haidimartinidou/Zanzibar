...
Use REAL songs (real artist + real title). When multiple styles are listed, BLEND them — don't just stack one then the other. Find natural bridges between styles. When a TIMELINE is provided, sequence tracks so each window's energy and style match what's requested at that point in the night. Assign each track a playSeconds cut-off using the rules above. Provide a one-line "reason" explaining why each track fits this exact moment of the set.`;
        ...
        tools: [{
          ...
          function: {
            ...
            parameters: {
              type: "object",
              properties: {
                name: { type: "string", description: "Catchy 2-5 word name for this set" },
                tracks: {
                  type: "array",
                  items: {
                    type: "object",
                    properties: {
                      ...
                      startMs: { type: "number", description: "Optional: source-track start in ms" },
                      endMs: { type: "number", description: "Optional: source-track end in ms" },
                      transitionToNext: { type: "string", enum: ["hard", "smooth"], description: "Cut style into the next track" },
                      transitionNote: { type: "string", description: "One-line description of the transition idea" },
                      globalEar: { type: "boolean", description: "True if this is a Global Ear discovery pick (non-English, international artist)" },
                      language: { type: "string", description: "Primary sung language of the track (e.g. 'Portuguese', 'Yoruba'). Required when globalEar is true." },
                    },
    ...
    const tracks = (parsed.tracks || []).map((t: any, i: number) => {
      ...
      return {
        ...
        ...(t.transitionToNext === "hard" || t.transitionToNext === "smooth"
          ? { transitionToNext: t.transitionToNext }
          : {}),
        ...(typeof t.transitionNote === "string" && t.transitionNote.trim()
          ? { transitionNote: t.transitionNote.trim() }
          : {}),
